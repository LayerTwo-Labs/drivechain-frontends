...
case "$platform" in 
    ...
    bitnames_version_postfix=pc-windows-gnu
    ...
    bitnames_version_postfix=unknown-linux-gnu
    unpack_cmd="unzip"
    ;;
    ...
    bitnames_version_postfix=apple-darwin
    unpack_cmd="tar -xf"
    ;;
...
bitnames=bitnamesd$bin_name_postfix
bitnames_cli=bitnames-cli$bin_name_postfix

# Avoid fetching the binary if it already exists
if ! test -f $assets_dir/$bitnames || ! test -f $assets_dir/$bitnames_cli; then
    echo bitnames binaries do not exist: $assets_dir/$bitnames and/or $assets_dir/$bitnames_cli

    # Fetch the bitnames binary for the specified platform
    bitnames_file=L2-S2-BitNames-latest-x86_64-$bitnames_version_postfix.zip

    curl --fail -O $RELEASES/$bitnames_file

    echo unpacking: $unpack_cmd $bitnames_file
    $unpack_cmd $bitnames_file

    # Use find to locate the bitnames binaries
    bitnames_binary=$(find . -name "bitnames-latest-*" -type f -print -quit)
    bitnames_cli_binary=$(find . -name "bitnames-cli-latest-*" -type f -print -quit)

    if [ -z "$bitnames_binary" ] || [ -z "$bitnames_cli_binary" ]; then
        echo "Error: Could not find one or both of the bitnames binaries in the extracted files."
        exit 1
    fi

    # Move the found binaries to the assets directory and rename them to ensure consistency
    mv "$bitnames_binary" "$assets_dir/$bitnames"
    mv "$bitnames_cli_binary" "$assets_dir/$bitnames_cli"

    # Check if the moves were successful
    if [ $? -ne 0 ]; then
        echo "Error: Failed to move the bitnames binaries. Please check the extracted directory structure."
        exit 1
    fi

    echo "bitnames binaries renamed to $bitnames and $bitnames_cli"

    # Make the binaries executable
    chmod +x "$assets_dir/$bitnames"
    chmod +x "$assets_dir/$bitnames_cli"
    echo "Made bitnames binaries executable"
fi
