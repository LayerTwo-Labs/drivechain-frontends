#!/usr/bin/env bash
#
# Thunder Installer Script
#
# Usage:
#   curl -sSL https://raw.githubusercontent.com/LayerTwo-Labs/drivechain-frontends/master/install/install-thunder.sh | bash
#   or
#   wget -qO- https://raw.githubusercontent.com/LayerTwo-Labs/drivechain-frontends/master/install/install-thunder.sh | bash
...
# Configuration
APPIMAGE_URL="https://releases.drivechain.info/Thunder-latest-x86_64-unknown-linux-gnu.AppImage"
APP_NAME="Thunder"
APP_NAME_LOWER="thunder"
INSTALL_DIR="$HOME/.local/bin"
ICON_URL="https://raw.githubusercontent.com/LayerTwo-Labs/drivechain-frontends/master/thunder/assets/thunder.png"
...
echo ""
echo -e "${BLUE}╔════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║      Thunder AppImage Installer        ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════╝${NC}"
echo ""
...
print_info "Downloading Thunder..."
...
Comment=Thunder Sidechain for Drivechain
Exec=$APPIMAGE_PATH %U
Terminal=false
Categories=Network;P2P;Finance;
StartupWMClass=thunder
Keywords=bitcoin;drivechain;sidechain;thunder;
EOF
...
ln -sf "$APPIMAGE_PATH" "$INSTALL_DIR/thunder"
...
echo -e "${GREEN}╚════════════════════════════════════════╝${NC}"
echo ""
echo -e "${GREEN}Thunder has been successfully installed!${NC}"
echo ""
echo "Installation details:"
echo "  - AppImage: $APPIMAGE_PATH"
echo "  - Desktop entry: $DESKTOP_FILE"
[ -n "$ICON_PATH" ] && echo "  - Icon: $ICON_PATH"
echo "  - Command: $INSTALL_DIR/thunder"
echo ""
echo "You can now:"
echo "  - Find Thunder in your application menu"
echo "  - Launch from terminal: $APPIMAGE_PATH"

if [ "$path_configured" = true ]; then
    echo "  - Run directly: thunder"
else
    echo ""
    print_warning "To run 'thunder' from anywhere, you need to add ~/.local/bin to your PATH"
    echo ""

    # Detect current shell
    current_shell=$(basename "$SHELL")

    echo "Run this command to add thunder to your PATH:"
    echo ""
...
echo ""
echo "To uninstall Thunder, run:"
echo "  rm $APPIMAGE_PATH"
echo "  rm $DESKTOP_FILE"
[ -n "$ICON_PATH" ] && echo "  rm $ICON_PATH"
echo "  rm $INSTALL_DIR/thunder"
echo ""
print_info "Enjoy using Thunder!"
