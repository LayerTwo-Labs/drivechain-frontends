#!/usr/bin/env bash
#
# ZSide Installer Script
#
# Usage:
#   curl -sSL https://raw.githubusercontent.com/LayerTwo-Labs/drivechain-frontends/master/install/install-zside.sh | bash
#   or
#   wget -qO- https://raw.githubusercontent.com/LayerTwo-Labs/drivechain-frontends/master/install/install-zside.sh | bash
...
# Configuration
APPIMAGE_URL="https://releases.drivechain.info/ZSide-latest-x86_64-unknown-linux-gnu.AppImage"
APP_NAME="ZSide"
APP_NAME_LOWER="zside"
INSTALL_DIR="$HOME/.local/bin"
ICON_URL="https://raw.githubusercontent.com/LayerTwo-Labs/drivechain-frontends/master/zside/assets/zside.png"
...
echo ""
echo -e "${BLUE}╔════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║       ZSide AppImage Installer         ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════╝${NC}"
echo ""
...
Comment=ZSide Sidechain for Drivechain
Exec=$APPIMAGE_PATH %U
Terminal=false
Categories=Network;P2P;Finance;
StartupWMClass=zside
Keywords=bitcoin;drivechain;sidechain;zside;zcash;privacy;
EOF
...
ln -sf "$APPIMAGE_PATH" "$INSTALL_DIR/zside"
...
echo -e "${GREEN}╚════════════════════════════════════════╝${NC}"
echo ""
echo -e "${GREEN}ZSide has been successfully installed!${NC}"
echo ""
echo "Installation details:"
echo "  - AppImage: $APPIMAGE_PATH"
echo "  - Desktop entry: $DESKTOP_FILE"
[ -n "$ICON_PATH" ] && echo "  - Icon: $ICON_PATH"
echo "  - Command: $INSTALL_DIR/zside"
echo ""
echo "You can now:"
echo "  - Find ZSide in your application menu"
echo "  - Launch from terminal: $APPIMAGE_PATH"

if [ "$path_configured" = true ]; then
    echo "  - Run directly: zside"
else
    echo ""
    print_warning "To run 'zside' from anywhere, you need to add ~/.local/bin to your PATH"
    echo ""

    # Detect current shell
    current_shell=$(basename "$SHELL")

    echo "Run this command to add zside to your PATH:"
    echo ""
...
echo ""
echo "To uninstall ZSide, run:"
echo "  rm $APPIMAGE_PATH"
echo "  rm $DESKTOP_FILE"
[ -n "$ICON_PATH" ] && echo "  rm $ICON_PATH"
echo "  rm $INSTALL_DIR/zside"
echo ""
print_info "Enjoy using ZSide!"
