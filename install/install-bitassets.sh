#!/usr/bin/env bash
#
# BitAssets Installer Script
#
# Usage:
#   curl -sSL https://raw.githubusercontent.com/LayerTwo-Labs/drivechain-frontends/master/install/install-bitassets.sh | bash
#   or
#   wget -qO- https://raw.githubusercontent.com/LayerTwo-Labs/drivechain-frontends/master/install/install-bitassets.sh | bash
...
# Configuration
APPIMAGE_URL="https://releases.drivechain.info/BitAssets-latest-x86_64-unknown-linux-gnu.AppImage"
APP_NAME="BitAssets"
APP_NAME_LOWER="bitassets"
INSTALL_DIR="$HOME/.local/bin"
ICON_URL="https://raw.githubusercontent.com/LayerTwo-Labs/drivechain-frontends/master/bitassets/assets/bitassets.png"
...
echo ""
echo -e "${BLUE}╔════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║     BitAssets AppImage Installer       ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════╝${NC}"
echo ""
...
print_info "Downloading BitAssets..."
...
Comment=BitAssets Sidechain for Drivechain - Digital assets platform
Exec=$APPIMAGE_PATH %U
Terminal=false
Categories=Network;P2P;Finance;
StartupWMClass=bitassets
Keywords=bitcoin;drivechain;sidechain;bitassets;assets;tokens;
EOF
...
ln -sf "$APPIMAGE_PATH" "$INSTALL_DIR/bitassets"
...
echo -e "${GREEN}╚════════════════════════════════════════╝${NC}"
echo ""
echo -e "${GREEN}BitAssets has been successfully installed!${NC}"
echo ""
echo "Installation details:"
echo "  - AppImage: $APPIMAGE_PATH"
echo "  - Desktop entry: $DESKTOP_FILE"
[ -n "$ICON_PATH" ] && echo "  - Icon: $ICON_PATH"
echo "  - Command: $INSTALL_DIR/bitassets"
echo ""
echo "You can now:"
echo "  - Find BitAssets in your application menu"
echo "  - Launch from terminal: $APPIMAGE_PATH"

if [ "$path_configured" = true ]; then
    echo "  - Run directly: bitassets"
else
    echo ""
    print_warning "To run 'bitassets' from anywhere, you need to add ~/.local/bin to your PATH"
    echo ""

    # Detect current shell
    current_shell=$(basename "$SHELL")

    echo "Run this command to add bitassets to your PATH:"
    echo ""
...
echo ""
echo "To uninstall BitAssets, run:"
echo "  rm $APPIMAGE_PATH"
echo "  rm $DESKTOP_FILE"
[ -n "$ICON_PATH" ] && echo "  rm $ICON_PATH"
echo "  rm $INSTALL_DIR/bitassets"
echo ""
print_info "Enjoy using BitAssets!"
