...
func sidechainTypeFromName(name string) pb.SidechainType {
	...
	case "freebank":
		// FreeBank is a plain-bitassets fork (the Rust freebankd), so it is a
		// CUSF chain the orchestrator manages exactly like bitassets — surface
		// it in the sync-status enumeration rather than dropping it.
...
func (h *Handler) GetSidechainBalance(ctx context.Context, req *connect.Request[pb.GetSidechainBalanceRequest]) (*connect.Response[pb.GetSidechainBalanceResponse], error) {
	name, displayName, err := sidechainNames(req.Msg.Sidechain)
	if err != nil {
...
// sidechainNames maps a sidechain binary to its config key and the name shown
// to the user.
func sidechainNames(binary pb.BinaryType) (name, displayName string, err error) {
	switch binary {
...
func applyMempoolDelta(confirmedSats, pendingSats int64, delta sidechain.MempoolDelta) (int64, int64) {
	...
	// A chained debit spends a coin the mempool made, so it nets against the
	// credit and never touches the confirmed count.
	return confirmedSats, max(pendingSats+delta.CreditSats-delta.ChainedDebitSats-owed, 0)
}
