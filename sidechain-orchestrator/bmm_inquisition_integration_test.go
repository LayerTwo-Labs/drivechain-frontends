...
//
// Run: INQUISITION_BITCOIND=... BIP300301_ENFORCER=... \
//
//	go test -tags integration -run TestInquisitionBMM ./...
func TestInquisitionBMM(t *testing.T) {
	bitcoind := os.Getenv("INQUISITION_BITCOIND")
	...
	inqDir := config.InquisitionDirs.RootDir()
	require.NoError(t, os.MkdirAll(inqDir, 0o700))
	startProcess(t, "inquisition", bitcoind,
		"-regtest", "-datadir="+inqDir, "-listen=0",
		fmt.Sprintf("-rpcport=%d", inquisitionPort),
		fmt.Sprintf("-sidechainslot=%d", inquisitionSlot),
		fmt.Sprintf("-enforcerport=%d", enforcerPort),
		"-fallbackfee=0.0001",
	)
	sidechain := newCoreRPC(t, config.InquisitionDirs.DatadirNetwork(config.NetworkRegtest, ""), inquisitionPort)
...
func activateSlot(ctx context.Context, t *testing.T, enforcer *jsonClient) {
	...
	for i := 0; i < 12; i++ {
		...
		for _, s := range active.Sidechains {
			if s.SidechainNumber == inquisitionSlot {
				return
			}
		}
	}
	t.Fatalf("slot %d never activated", inquisitionSlot)
}
