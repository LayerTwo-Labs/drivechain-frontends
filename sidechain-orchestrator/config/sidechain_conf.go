...
config-version=%d
`, m.Spec.Name, m.Spec.Name, ports["rpc-port"], ports["net-addr"], ports["zmq-addr"], SidechainConfMigrationsVersion)
...
config-version=%d
`, m.Spec.Name, m.Spec.Name, ports["rpc-addr"], ports["net-addr"], ports["mainchain-grpc-url"], SidechainConfMigrationsVersion)
	}
}
