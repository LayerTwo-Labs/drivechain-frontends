...
func (m *EnforcerConfManager) GetCliArgs() []string {
	...
	pinned := seen["wallet-esplora-url"] || seen["wallet-sync-source"] ||
		seen["wallet-electrum-host"] || seen["wallet-electrum-port"]
	if host, port := ElectrumHostPortForNetwork(m.bitcoinConf.Network); host != "" && !pinned {
		args = append(args,
			"--wallet-sync-source=electrum",
			fmt.Sprintf("--wallet-electrum-host=%s", host),
			fmt.Sprintf("--wallet-electrum-port=%d", port),
		)
	} else if !seen["wallet-esplora-url"] {
