...
func TestBuildSendOutputsRawOutputsFirst(t *testing.T) {
	treasuryHex := hex.EncodeToString([]byte{0xB7, 0x01, 0x01, 0x51})
	req := SendRequest{
...
func TestOutputToTxOutRawScript(t *testing.T) {
	...
	script := []byte{0xB7, 0x01, 0x01, 0x51}
...
// TestM8TreasuryScriptAllSlots verifies the OP_DRIVECHAIN treasury script is
...
func TestM8TreasuryScriptAllSlots(t *testing.T) {
	for slot := 0; slot <= 255; slot++ {
		want := []byte{0xB7, 0x01, byte(slot), 0x51}
		out, err := outputToTxOut(
...
// TestM8OpReturnAddressRoundtrip verifies the sidechain address survives the
// OP_RETURN encoding intact — mirrors the enforcer's try_parse_op_return_address.
func TestM8OpReturnAddressRoundtrip(t *testing.T) {
...
func TestBuildPSBTExternalInputPrefinalized(t *testing.T) {
	...
	drivechainScript := []byte{0xB7, 0x01, 0x01, 0x51}
