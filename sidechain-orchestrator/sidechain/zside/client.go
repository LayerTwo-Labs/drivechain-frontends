...
// Client is a JSON-RPC client for the Zside sidechain node.
// It mirrors the RPC surface exposed in the Dart ZsideRPC class.
type Client struct {
	baseURL string
	http    *http.Client
}

// NewClient creates a Zside RPC client pointed at host:port.
...
func (e *rpcError) Error() string {
	return fmt.Sprintf("zside RPC error %d: %s", e.Code, e.Message)
}
...
// Stop gracefully shuts down the Zside node.
