...
// Client is a JSON-RPC client for the Coinshift sidechain node.
// It mirrors the RPC surface exposed in the Dart CoinshiftRPC class.
type Client struct {
	baseURL string
	http    *http.Client
}

// NewClient creates a Coinshift RPC client pointed at host:port.
...
func (e *rpcError) Error() string {
	return fmt.Sprintf("coinshift RPC error %d: %s", e.Code, e.Message)
}
...
// Stop gracefully shuts down the Coinshift node.
