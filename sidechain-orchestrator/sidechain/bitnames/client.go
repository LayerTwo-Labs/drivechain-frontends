...
// Client is a JSON-RPC client for the BitNames sidechain node.
// It mirrors the RPC surface exposed in the Dart BitnamesRPC class.
type Client struct {
	baseURL string
	http    *http.Client
}
...
func (e *rpcError) Error() string {
	return fmt.Sprintf("bitnames RPC error %d: %s", e.Code, e.Message)
}
...
func (c *Client) BitNameData(ctx context.Context, name string) (*BitNameData, error) {
	...
	if err := json.Unmarshal(raw, &d); err != nil {
		return nil, fmt.Errorf("decode bitname_data result: %w", err)
	}
	return &d, nil
}
...
func (c *Client) RegisterBitName(ctx context.Context, plainName string, data *BitNameData) (string, error) {
	return unmarshal[string](c, ctx, "register_bitname", []interface{}{plainName, data})
}

// ReserveBitName reserves a BitName hash.
func (c *Client) ReserveBitName(ctx context.Context, name string) (string, error) {
	return unmarshal[string](c, ctx, "reserve_bitname", []interface{}{name})
}
...
func (c *Client) GetNewEncryptionKey(ctx context.Context) (string, error) {
	return unmarshal[string](c, ctx, "get_new_encryption_key", nil)
}
...
func (c *Client) GetNewVerifyingKey(ctx context.Context) (string, error) {
	return unmarshal[string](c, ctx, "get_new_verifying_key", nil)
}
...
func (c *Client) EncryptMsg(ctx context.Context, encryptionPubkey, msg string) (string, error) {
	return unmarshal[string](c, ctx, "encrypt_msg", []interface{}{encryptionPubkey, msg})
}
