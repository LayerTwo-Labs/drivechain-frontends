...
// Client is a JSON-RPC client for the Bitassets sidechain node.
// It mirrors the RPC surface exposed in the Dart BitassetsRPC class.
type Client struct {
	baseURL string
	http    *http.Client
}

// NewClient creates a Bitassets RPC client pointed at host:port.
...
func (e *rpcError) Error() string {
	return fmt.Sprintf("bitassets RPC error %d: %s", e.Code, e.Message)
}
...
// Stop gracefully shuts down the Bitassets node.
