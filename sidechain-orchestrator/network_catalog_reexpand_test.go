...
func TestAdoptCatalogReExpandsFromRawConfig(t *testing.T) {
	...
	o.ecashID = "drynet2"
	o.configs[raw.Name] = expandECashPlaceholder(o.rawConfigs[raw.Name], "drynet2")
	if got := o.configs[raw.Name].Variants["ecash"].Subfolder; got != "drynet2" {
	...
	for name, pristine := range o.rawConfigs {
		o.configs[name] = expandECashPlaceholder(pristine, "drynet3")
	}
	o.mu.Unlock()

	if got := o.configs[raw.Name].Variants["ecash"].Subfolder; got != "drynet3" {
		t.Errorf("subfolder after re-expansion = %q, want drynet3", got)
	}
	want := "L1-ecash-bitcoin-drynet3-x86_64-unknown-linux-gnu.zip"
	if got := o.configs[raw.Name].Variants["ecash"].Files["linux-x86_64"]; got != want {
