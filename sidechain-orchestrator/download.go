...
func (d *DownloadManager) fetchArchive(ctx context.Context, config BinaryConfig, target DownloadTarget, tmpDir string, send func(DownloadProgress) bool) (string, error) {
	...
	if config.RequireHash && (!pinned || expected.SHA256 == "") {
		return "", fmt.Errorf("no trusted archive hash for %s", config.Name)
	}
	...
	if pinned {
		...
		if !strings.EqualFold(expected.SHA256, archiveHash) {
			return "", fmt.Errorf("archive hash mismatch for %s", config.Name)
		}
		if expected.Size > 0 && expected.Size != archiveSize {
			return "", fmt.Errorf("archive size mismatch for %s", config.Name)
		}
	}
	if err != nil {
