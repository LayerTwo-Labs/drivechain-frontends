...
func (d *DownloadManager) DownloadWithOptions(ctx context.Context, config BinaryConfig, network string, force bool, opts DownloadOptions) (<-chan DownloadProgress, error) {
	if err := checkArtifactPins(config, currentPlatform()); err != nil {
		return nil, err
	}
	targets := d.Targets(config, network, opts)
	binPath := targets[0].BinPath
	if !force && len(config.ArtifactPins) > 0 {
		if _, err := os.Lstat(binPath); err == nil {
			if err := verifyArtifactPin(config, binPath, false); err != nil {
				return nil, err
			}
		}
	}
...
func (d *DownloadManager) install(ctx context.Context, config BinaryConfig, network string, claims []*inFlightClaim, send func(DownloadProgress) bool) (err error) {
	...
	for i, claim := range claims {
		...
		if err := verifyArtifactPin(config, claim.target.BinPath, false); err != nil {
			return err
		}
	}
	return nil
...
func (d *DownloadManager) fetchArchive(ctx context.Context, config BinaryConfig, target DownloadTarget, tmpDir string, send func(DownloadProgress) bool) (string, error) {
	...
	if len(config.ArtifactPins) > 0 {
		if err := verifyArtifactPin(config, savePath, true); err != nil {
			return "", err
		}
		return savePath, nil
	}
