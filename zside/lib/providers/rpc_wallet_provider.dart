...
/// RPC-backed wallet reader that delegates to the Go wallet service via ZSidedRPC.
/// Used by ZSide instead of the filesystem-based WalletReaderProvider.
class RpcWalletReaderProvider extends ChangeNotifier {
  final ZSidedRPC _zsidedRPC;
  ...
  RpcWalletReaderProvider(this._zsidedRPC, this.bitwindowAppDir);
  ...
  Future<bool> unlockWallet(String password) async {
    try {
      await _zsidedRPC.walletUnlock(password);
  ...
  Future<void> encryptWallet(String password) async {
    await _zsidedRPC.walletEncrypt(password);
    unlockedPassword = password;
    await _refreshStatus();
    notifyListeners();
  }
  ...
  Future<void> changePassword(String oldPassword, String newPassword) async {
    await _zsidedRPC.walletChangePassword(oldPassword, newPassword);
    unlockedPassword = newPassword;
    notifyListeners();
  }

  Future<void> removeEncryption(String password) async {
    await _zsidedRPC.walletRemoveEncryption(password);
    unlockedPassword = null;
    await _refreshStatus();
    notifyListeners();
  }
  ...
  Future<void> switchWallet(String walletId) async {
    await _zsidedRPC.walletSwitch(walletId);
    activeWalletId = walletId;
    notifyListeners();
  }

  Future<void> updateWalletMetadata(String walletId, String name, WalletGradient gradient) async {
    await _zsidedRPC.walletUpdateMetadata(walletId, name, gradient.toJsonString());
    await _refreshStatus();
    notifyListeners();
  }

  Future<void> removeWalletFromList(String walletId) async {
    await _zsidedRPC.walletDelete(walletId);
    await _refreshStatus();
    notifyListeners();
  }
  ...
  Future<void> _refreshStatus() async {
    try {
      final status = await _zsidedRPC.getWalletStatus();
  ...
  Future<void> _refreshWalletList() async {
    try {
      final response = await _zsidedRPC.walletList();
