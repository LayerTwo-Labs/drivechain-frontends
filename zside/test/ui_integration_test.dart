  ...
  group('UI Component Tests', () {
    testWidgets('SailCard renders with title and content', (tester) async {
      await tester.pumpSailPage(
        SailCard(
          title: 'ZSide Card',
          subtitle: 'Privacy-focused sidechain',
          child: SailText.primary15('Card Content'),
        ),
      );
      await tester.pumpAndSettle();

      expect(find.text('ZSide Card'), findsOneWidget);
      expect(find.text('Privacy-focused sidechain'), findsOneWidget);
    ...
    testWidgets('SailTextField accepts input', (tester) async {
      ...
      await tester.pumpSailPage(
        SailTextField(
          ...
          hintText: 'Enter z-address',
        ),
      );
      await tester.pumpAndSettle();

      await tester.enterText(find.byType(TextField), 'zs1test123');
      expect(controller.text, 'zs1test123');
    });
  });
      ...
      await tester.pumpSailPage(
        StatefulBuilder(
          builder: (context, setState) => SailDropdownButton<String>(
            value: 'shielded',
            items: const [
              SailDropdownItem(value: 'shielded', label: 'Shielded'),
              SailDropdownItem(value: 'transparent', label: 'Transparent'),
            ],
            onChanged: (value) {},
          ),
        ),
