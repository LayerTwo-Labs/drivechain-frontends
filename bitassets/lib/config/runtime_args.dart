...
abstract class RuntimeArgs {
  ...
  static Future<Directory> datadir() async {
    final fromEnv = Platform.environment['BITASSETS_DATADIR'] ?? const String.fromEnvironment('BITASSETS_DATADIR');
  ...
  static bool consoleLog = Platform.environment['BITASSETS_LOG_CONSOLE']?.isNotEmpty ?? false;

  static bool fileLog = Platform.environment['BITASSETS_LOG_FILE']?.isNotEmpty ?? false;

  static final String _network =
      Platform.environment['BITASSETS_NETWORK'] ?? const String.fromEnvironment('BITASSETS_NETWORK');
