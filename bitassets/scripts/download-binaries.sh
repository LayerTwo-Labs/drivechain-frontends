...
case "$platform" in 
    ...
    bitassets_version_postfix=pc-windows-gnu
    ...
    bitassets_version_postfix=unknown-linux-gnu
    unpack_cmd="unzip"
    ;;
    ...
    bitassets_version_postfix=apple-darwin
    unpack_cmd="tar -xf"
    ;;
...
bitassets=bitassetsd$bin_name_postfix
bitassets_cli=bitassets-cli$bin_name_postfix
...
if ! test -f $assets_dir/$bitassets || ! test -f $assets_dir/$bitassets_cli; then
    ...
    curl --fail -O $RELEASES/$bitassets_file

    echo unpacking: $unpack_cmd $bitassets_file
    $unpack_cmd $bitassets_file

    # Use find to locate the bitassets binaries
    bitassets_binary=$(find . -name "bitassets-latest-*" -type f -print -quit)
    bitassets_cli_binary=$(find . -name "bitassets-cli-latest-*" -type f -print -quit)

    if [ -z "$bitassets_binary" ] || [ -z "$bitassets_cli_binary" ]; then
        echo "Error: Could not find one or both of the bitassets binaries in the extracted files."
        exit 1
    fi

    # Move the found binaries to the assets directory and rename them to ensure consistency
    mv "$bitassets_binary" "$assets_dir/$bitassets"
    mv "$bitassets_cli_binary" "$assets_dir/$bitassets_cli"

    # Check if the moves were successful
    if [ $? -ne 0 ]; then
        echo "Error: Failed to move the bitassets binaries. Please check the extracted directory structure."
        exit 1
    fi
    ...
    chmod +x "$assets_dir/$bitassets_cli"
    echo "Made bitassets binaries executable"
fi
