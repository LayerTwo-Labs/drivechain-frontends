...
import { FaucetService } from "@/gen/hub/v1/faucet_pb";
import { timestampToDate } from "@/lib/api";
import { clientTransport } from "@/lib/client-api";
import { blockExplorerUrl, exampleAddress, faucetEnabled } from "@/lib/network";
...
export default function Home() {
  ...
  const MAX_AMOUNT = network.services.faucet.amount ?? 5;
  const ticker = network.currency.ticker;
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            ...
            {available !== null &&
              !faucetUnhealthy &&
              ` — ${available.toFixed(8)} ${ticker} available`}
          </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <Input
                id="address"
                placeholder={`Enter a L1-address (e.g. ${exampleAddress(network)})`}
                value={address}
          ...
          {!faucetUnhealthy && insufficientFunds && (
            <div className="p-3 text-sm text-amber-600 bg-amber-50 rounded-md border border-amber-200">
              The faucet only has {available?.toFixed(8)} {ticker} available right now. Lower the
          ...
          {txid && (
            <div className="p-3 text-sm text-green-700 bg-green-50 rounded-md border border-green-200">
              Dispensed {amount} {ticker}!{" "}
              <a
                href={blockExplorerUrl(network, txid) ?? undefined}
                target="_blank"
                    ...
                    <tr key={claim.txid} className="bg-background border-b">
                      ...
                      <td className="px-3 sm:px-6 py-4 font-mono text-xs">
                        <div className="truncate max-w-[80px] sm:max-w-[200px] md:max-w-full">
                          <a
                            href={blockExplorerUrl(network, claim.txid) ?? undefined}
                            target="_blank"
