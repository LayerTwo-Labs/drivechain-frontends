...
func TestFastRelaunchKeepsDrivechaindAlive(t *testing.T) {
	...
	t.Logf("first launch: app tree exited, surviving bitwindowd=%s drivechaind=%s",
		prettyPIDs(survivors), prettyPIDs(processPIDs(t, drivechaindName)))
	...
	fresh := waitForNewPID(t, bitwindowdName, survivors, bootDeadline, bootPoll,
		"second launch: no new bitwindowd appeared")
	t.Logf("second launch: bitwindowd=%s", prettyPIDs(fresh))
