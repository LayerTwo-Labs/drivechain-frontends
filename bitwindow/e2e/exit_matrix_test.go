...
func assertRestartAfterExit(t *testing.T, path exitPath) {
	...
	// A daemon of the first launch may still wind down, and a bare "one is
	// running" check would pass on it and prove nothing about the relaunch.
	survivors := processPIDs(t, bitwindowdName)
	...
	fresh := waitForNewPID(t, bitwindowdName, survivors, bootDeadline, bootPoll,
		path.name+": no new bitwindowd appeared after the relaunch")
	t.Logf("%s: second launch bitwindowd=%s", path.name, prettyPIDs(fresh))
