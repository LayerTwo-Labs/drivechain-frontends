...
class BitMessageServer {
  ...
  final int port, maxBodyBytes;
  final Duration requestBodyTimeout, callbackTimeout, idleTimeout;
  ...
  Future<Uri> start() async {
    if (isRunning) {
      throw StateError('BitMessageServer is already running');
    }
  ...
  Future<void> _serveProfile(HttpResponse response, String? hash) async {
    ...
    if (profile == null) {
      return _json(response, HttpStatus.notFound, {'error': 'identity_not_found'});
    }
