...
class BitMessageServer {
  BitMessageServer({
    ...
    if (maxBodyBytes <= 0) throw ArgumentError.value(maxBodyBytes, 'maxBodyBytes', 'must be positive');
    if (port < 0 || port > 65535) throw ArgumentError.value(port, 'port', 'must be between 0 and 65535');
    if (maxConcurrentRequests < 1) throw ArgumentError.value(maxConcurrentRequests, 'maxConcurrentRequests');
  }

  final BitMessageProfileProvider profileProvider;
  final IncomingBitMessageCallback onIncomingWire;
  final bool Function()? allowRemoteClients;
