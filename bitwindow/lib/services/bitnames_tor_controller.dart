...
class BitnamesTorStatus {
  const BitnamesTorStatus({
    required this.state,
    this.onionHost,
    this.p2pOnion,
    this.p2pReady = false,
    this.socksAddress = '127.0.0.1:19050',
    this.error,
  });
...
class BitnamesTorController {
  ...
  Future<BitnamesTorStatus> downloadAndStart({Duration timeout = const Duration(minutes: 15)}) async {
    if ((await orchestrator.getBinaryStatus(binaryName)).status.downloaded) {
      return start();
    }
    ...
    while (DateTime.now().isBefore(deadline)) {
      ...
      if (status.downloaded) {
        return start(timeout: const Duration(minutes: 2));
      }
  ...
  Future<BitnamesTorStatus> start({Duration timeout = const Duration(minutes: 2)}) async {
    ...
    while (DateTime.now().isBefore(deadline)) {
      ...
      if (status?.state == BitnamesTorState.error) {
        throw StateError(status?.error ?? 'BitNames Tor failed to start');
      }
      await _pause();
    }
  ...
  Future<void> stop() async {
    ...
    await orchestrator.stopBinary(binaryName);
  }
  ...
  Future<void> enableChainTor(
    ...
    if (!status.ready || !status.p2pReady) {
      throw StateError('Start BitNames Tor before enabling Tor mode');
    }
  ...
  Future<void> disableChainTor({Duration timeout = const Duration(minutes: 2)}) async {
    _chainTorPrepared = false;
    _tunnelAddresses.clear();
    await _restartBitNames(const [], timeout);
  }

  Future<bool> chainTorReady(BitnamesRPC rpc) async {
    if (!_chainTorPrepared) {
      return false;
    }
    final status = await _runtimeStatus();
    if (!(status?.ready ?? false) || !(status?.p2pReady ?? false)) {
      return false;
    }
    try {
  ...
  Future<void> _restartBitNames(List<String> args, Duration timeout) async {
    await orchestrator.stopBinary('bitnames', force: true);
    final deadline = DateTime.now().add(timeout);
    while ((await orchestrator.getBinaryStatus('bitnames')).status.connected && DateTime.now().isBefore(deadline)) {
      await _pause();
    }
    await _pause();
  ...
  Future<BitnamesTorStatus?> _runtimeStatus() async {
    try {
      ...
      if (json['ready'] == true) {
        state = BitnamesTorState.ready;
      }
      return BitnamesTorStatus(
        state: state,
        onionHost: _withoutScheme(json['onion'] as String?),
        p2pOnion: json['p2p_onion'] as String?,
        p2pReady: json['p2p_ready'] == true,
        socksAddress: json['socks_address'] as String? ?? '127.0.0.1:19050',
        error: error,
      );
    } catch (_) {
      return null;
