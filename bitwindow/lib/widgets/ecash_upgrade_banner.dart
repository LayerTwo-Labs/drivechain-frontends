...
class _ECashUpgradeDialogState extends State<ECashUpgradeDialog> {
    ...
    return SailDialog(
      ...
      child: SailColumn(
        ...
        children: [
          ...
          if (p.userManagedConf) ..._manualSteps(p, blocked),
  ...
  /// The steps a user with their own bitcoin.conf follows. They appear only
  /// with a plan that names the rollback block: without it these steps boot the
  /// target over the source fork's chain.
  List<Widget> _manualSteps(GetPendingNetworkGenerationResponse p, bool blocked) {
    if (blocked) {
      return [
        SailText.primary13(
          'This switch cannot run: neither network publishes the fork height that says where the '
          'chains part, so nothing can roll back to a block they share.',
        ),
      ];
    }
    final rewindHeight = _plan?.rewindHeight ?? 0;
    if (rewindHeight == 0) {
      return [SailText.primary13('Reading the switch plan for ${p.pendingNetworkId}…')];
    }
    return [
      SailText.primary13(
      ...
      SailText.secondary13(
        '• Roll the chain back to block $rewindHeight from the Chain tab. Keep blocks/ and '
