...
class _WelcomeModalContentState extends State<_WelcomeModalContent> {
  ...
  Future<void> _handleFastMode() async {
    await _walletProvider.generateWallet().then((wallet) {
      if (!mounted) return;
      ...
      _walletProvider.saveWallet(wallet).then((success) {
        if (!mounted) return;
        ...
        _walletProvider.generateStartersForDownloadedChains().then((_) {
          if (!mounted) return;
