...
Future<ReplayChoice> askReplayProtection(BuildContext context, {required List<UnspentOutput> coins}) async {
  final choice = await showThemedDialog<ReplayChoice>(
    context: context,
    builder: (context) => SailModal(
      constraints: const BoxConstraints(maxWidth: 720),
      child: SailCard(
        title: coins.length == 1 ? 'This coin exists on both chains' : 'These coins exist on both chains',
        subtitle: 'Spending them makes the same transaction on the other chain.',
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            SailText.primary15('Coins on both chains', bold: true),
            const SizedBox(height: SailStyleValues.padding08),
            ...coins.map((c) => _CoinRow(coin: c)),
