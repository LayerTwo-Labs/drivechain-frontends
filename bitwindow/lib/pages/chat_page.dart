...
class ChatViewModel extends BaseViewModel {
  ...
  Future<void> sendMessage(BuildContext context) async {
    ...
    if (context.mounted) {
      await _handleFallback(context, result);
    }
  }
  ...
  Future<void> _sendFallback(BuildContext context, String id) async {
    ...
    if (!result.sent && context.mounted) {
      showSailToast(context, result.error ?? 'Chain send failed');
    }
  }
    ...
    await _runPrivateAction(
      ...
      action: () async {
        final path = await FilePicker.saveFile(
          ...
          bytes: utf8.encode(await _chatProvider.exportPrivateData()),
  ...
  Future<void> _runPrivateAction(
    ...
    try {
      ...
      if (completed && context.mounted) {
        showSailToast(context, success, variant: SailToastVariant.success);
      }
    } catch (e) {
      if (context.mounted) {
        showSailToast(context, '$failure: $e');
      }
    }
  }
