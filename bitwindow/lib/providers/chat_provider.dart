...
class ChatSendResult {
  const ChatSendResult(this.disposition, {this.id, this.reference, this.error});
  final ChatSendDisposition disposition;
  final String? id, reference, error;
  bool get sent => disposition == ChatSendDisposition.sent;
...
class ChatProvider extends ChangeNotifier {
  ChatProvider({
    ...
    _secureStore =
        secureStore ??
        BitnamesSecureStore(
          store: _settings.store is FileStorage
              ? ChatFileStorage.fromDirectory((_settings.store as FileStorage).file.parent)
              : _settings.store,
          keySource: _storageKeys,
        );
    _verifier = BitnamesBitMessageProfileVerifier(
      bitnamesRPC,
      commitmentNetwork: _commitmentNetwork,
    );
    _codec = BitIntroductionEnvelopeCodec(
      signer: BitnamesBitIntroductionSigner(bitnamesRPC),
      signatureBackend: BitnamesBitIntroductionSignatureVerifier(bitnamesRPC),
    ...
    _server = BitMessageServer(
      ...
      onIncomingWire: (wire) async {
        if (!await receiveWire(wire, ChatTransport.direct)) {
          throw const FormatException('BitMessage was rejected');
        }
      },
    );
  ...
  static const _namespacedCommitmentsActivated = bool.fromEnvironment('BITNAMES_NAMESPACED_COMMITMENTS');
  final BitnamesRPC bitnamesRPC;
  final ClientSettings _settings;
  final BalanceProvider? _balances;
  ...
  late final BitnamesBitMessageProfileVerifier _verifier;
  late final BitIntroductionEnvelopeCodec _codec;
  late final BitMessageTransport _transport;
  late final BitMessageServer _server;
  ...
  final Map<String, BitMessageProfile> _profiles = {};
  final Set<String> _pendingProfiles = {};
  final Map<String, BitMessageWire> _pendingWires = {};
  ...
  bool get hasOlderMessages => _allCurrentConversation.length > currentConversation.length;
  ...
  void _hydrate(Map<String, dynamic> state) {
    ...
    for (final json in _maps(state['profiles'])) {
      final profile = BitMessageProfile.fromJson(json);
      _profiles[profile.bitNameHash] = profile;
    }
  ...
  Future<void> _startRuntime() async {
    if (!_server.isRunning) {
      try {
        ...
        _error = 'BitMessage listener unavailable: $e';
      }
    }
    startPolling();
  }
  ...
  void _onStorageKeyChanged() {
    unawaited(
      _storageReloadLock.synchronized(() async {
        if (_disposed) {
          return;
        }
        var ready = false;
  ...
  Future<bool> _refreshChainHealth() async {
    ...
    try {
      ...
      final bitnamesMainchainHash = await bitnamesRPC.getBestMainchainBlockHash();
  ...
  bool get _hasPendingChainWork =>
      ...
      _messages.any(
        (message) =>
            message.transport == ChatTransport.bitnamesChain && message.deliveryState == ChatDeliveryState.pending,
      );
  ...
  String _operationStatus(BitnamesOperation operation) {
    final name =
        ...
        (operation.bitnameHash.length > 8 ? '${operation.bitnameHash.substring(0, 8)}…' : operation.bitnameHash);
    return switch (operation.phase) {
      BitnamesOperationPhase.prepared => 'Preparing BitName operation for "$name"',
      ...
      BitnamesOperationPhase.profileSubmitted =>
        'Reply profile for "$name" submitted • waiting for a BitNames block'
            '${operation.txid == null ? '' : ' • transaction ${_short(operation.txid!)}'}',
      BitnamesOperationPhase.paused => '$name operation paused safely • ${operation.lastError ?? 'review required'}',
      BitnamesOperationPhase.failed => '$name operation failed • ${operation.lastError ?? 'unknown error'}',
  ...
  Future<void> fetchIdentities() async {
    ...
    try {
      ...
      _contacts = _contacts
          .map(
            (contact) =>
                namesByHash[contact.id] == null ? contact : contact.copyWith(plaintextName: namesByHash[contact.id]),
          )
          .toList();
  ...
  Future<String?> publishProfile({
    ...
    if (me == null) {
      _fail('Select a registered BitName first');
      return null;
    }
    ...
    if (paymailFeeSats != null && paymailFeeSats < 0) {
      _fail('Introduction fee cannot be negative');
      return null;
    }
    ...
    try {
      ...
      final proposedProfile = BitMessageProfile(
        bitNameHash: identity.hash,
        signingPublicKey: signing,
        encryptionPublicKey: encryption,
        directEndpoints: direct.map((u) => _path(u, identity.hash)),
        torEndpoints: tor.map((u) => _path(u, identity.hash)),
      ...
      final txid = await bitnamesRPC.updateBitName(
        bitname: identity.hash,
        updates: BitNameDataUpdates(
          commitment: BitNameUpdate.set(plan.commitment),
          signingPubkey: BitNameUpdate.set(signing),
          encryptionPubkey: BitNameUpdate.set(encryption),
          paymailFeeSats: paymailFeeSats == null
              ? const BitNameUpdate<int>.delete()
              : BitNameUpdate.set(paymailFeeSats),
        ),
        feeSats: minerFeeSats,
      );
      _profiles[identity.hash] = profile;
      _pendingProfiles.add(identity.hash);
  ...
  Future<bool> addContactFromEntry(BitnameEntry entry) async {
    if (entry.hash == _selectedIdentity?.hash) {
      _fail('Choose a different BitName; you cannot add your current identity');
      return false;
    }
    ...
    if (resolution == null) {
      _fail('Could not resolve this BitName');
      return false;
    }
  ...
  Future<ChatContact?> lookupBitName(String nameOrHash) async {
    ...
    try {
      ...
      if (data == null || data.encryptionPubkey == null || _selectedIdentity == null) {
        return null;
      }
  ...
  Future<void> addContact(ChatContact contact) async {
    ...
    if (existingIndex >= 0) {
      ...
      contact = ChatContact(
        id: contact.id,
        localBitname: contact.localBitname,
        name: contact.name,
        plaintextName: contact.plaintextName,
        encryptionPubkey: contact.encryptionPubkey,
        signingPubkey: contact.signingPubkey,
        address: contact.address,
        paymailFeeSats: contact.paymailFeeSats,
        relationshipState: old.relationshipState,
        introductionId: old.introductionId,
        replyProfile: old.replyProfile,
    ...
    try {
      ...
      if (!alreadyRegistered) {
        try {
          ...
          if (!e.toString().toLowerCase().contains('missing bitname')) {
            rethrow;
          }
        }
      }
      if (alreadyRegistered && operation == null) {
        _fail('That BitName is already registered');
        return null;
      }
      if (operation == null) {
  ...
  Future<void> _reconcileOperations() => _operationLock.synchronized(_reconcileOperationsUnlocked);

  Future<void> _reconcileOperationsUnlocked() async {
    if (!bitnamesRPC.connected || !_chainHealth.mutationSafe) {
      return;
    }
    for (final operation in _operations.values.toList()) {
      if (operation.phase == BitnamesOperationPhase.failed || operation.phase == BitnamesOperationPhase.cancelled) {
        continue;
      }
  ...
  Future<void> _reconcileRegistration(BitnamesOperation operation) async {
    ...
    switch (operation.phase) {
      ...
      case BitnamesOperationPhase.reservationSubmitting:
        if (operation.lastError == null) {
          await _transitionOperation(
            ...
            (_chainHealth.sidechainHeight ?? 0) > operation.lastObservedSidechainHeight!) {
      ...
      case BitnamesOperationPhase.reservationSubmitted:
        ...
        if (status == BitnamesTransactionStatus.pending) {
          return;
        }
  ...
  Future<void> _submitRegistration(
    ...
    if (current.encryptionPubkey == null || current.signingPubkey == null) {
      current = current.copyWith(
        encryptionPubkey: current.encryptionPubkey ?? await bitnamesRPC.getNewEncryptionKey(),
        signingPubkey: current.signingPubkey ?? await bitnamesRPC.getNewVerifyingKey(),
        updatedAt: _now(),
      );
  ...
  Future<void> _reconcileProfile(BitnamesOperation operation) async {
    ...
    final identity = _allBitNames.where((entry) => entry.hash == operation.bitnameHash).firstOrNull;
    final canonical = identity != null && _profileMatchesCommitment(identity, profile);
    if (canonical) {
    ...
    if (operation.phase != BitnamesOperationPhase.profileSubmitted) {
      return;
    }
    final txid = operation.txid;
    if (txid == null) {
  ...
  Future<ChatSendResult> sendIntroduction(String body) async {
    ...
    if (_isSending) {
      return _failed('A message is already being sent');
    }
    ...
    try {
      ...
      if (fee != ctx.contact.paymailFeeSats) {
        await addContact(
          ChatContact(
            id: ctx.contact.id,
            localBitname: ctx.contact.localBitname,
            name: ctx.contact.name,
            plaintextName: ctx.contact.plaintextName,
            encryptionPubkey: ctx.remote.data.encryptionPubkey!,
            signingPubkey: ctx.remote.data.signingPubkey,
            address: ctx.remote.address,
            paymailFeeSats: fee,
            isManual: ctx.contact.isManual,
          ),
        );
        if (fee == null) {
          return _failed('Recipient is not accepting introductions');
        }
      ...
      final id = prepared.$1.payload.id,
          pending = _message(
            prepared.$1,
            true,
            ChatTransport.bitnamesChain,
            null,
            fee,
          ).copyWith(deliveryState: ChatDeliveryState.pending);
      _pendingWires[id] = prepared.$2;
      final contact = ctx.contact.copyWith(
        address: ctx.remote.address,
        relationshipState: ChatRelationshipState.outgoingIntroduction,
        introductionId: prepared.$1.payload.id,
      );
      _storeContactInMemory(contact);
  ...
  Future<ChatSendResult> _sendDirect(String body, BitIntroductionKind kind, String? reply) async {
    ...
    final ctx = await _context();
    final accepting =
        kind == BitIntroductionKind.acceptance &&
        ctx?.contact.relationshipState == ChatRelationshipState.incomingIntroduction;
    if (ctx == null) {
      return _failed(_error ?? 'Publish the reply profile in Messaging setup and wait for its BitNames block');
    }
    if (!ctx.contact.isAccepted && !accepting) {
      return _failed('Chat is not accepted');
    }
    final remoteProfile = _profile(ctx.contact);
    if (remoteProfile == null) {
      return _failed('This contact has not published a verified reply profile');
    }
    late (SignedBitIntroductionEnvelope, BitMessageWire) prepared;
    try {
      prepared = await _prepare(ctx, body, kind, reply, true);
    } catch (e) {
    ...
    try {
      ...
      try {
        ...
        verified = await _verifier.verify(current);
        await _putContact(
          ctx.contact.copyWith(
            encryptionPubkey: current.encryptionPublicKey,
            signingPubkey: current.signingPublicKey,
            replyProfile: current.toJson(),
          ),
        );
      }
  ...
  Future<ChatSendResult> acceptIntroduction() async {
    ...
    final contact = _selectedContact;
    if (contact?.relationshipState != ChatRelationshipState.incomingIntroduction) {
      return _failed('No introduction to accept');
    }
    if (_messages.any(
      (m) =>
          m.kind == ChatMessageKind.acceptance &&
          m.senderBitname == contact!.localBitname &&
          m.recipientBitname == contact.id &&
          _pendingWires.containsKey(m.id),
    )) {
    ...
    _changed();
    try {
      return await _sendDirect('accepted', BitIntroductionKind.acceptance, contact!.introductionId);
    } finally {
      _isSending = false;
      _changed();
    }
  ...
  Future<void> cancelIntroduction() async {
    final contact = _selectedContact;
    if (_isSending || contact?.relationshipState != ChatRelationshipState.outgoingIntroduction) {
      return;
    }
    ...
    await _putContact(contact.copyWith(relationshipState: ChatRelationshipState.rejected, clearIntroductionId: true));
    await _save();
  }
  ...
  Future<void> _setRelationship(ChatRelationshipState state) async {
    if (!_isSending && _selectedContact != null) {
      await _putContact(_selectedContact!.copyWith(relationshipState: state));
    }
  }
  ...
  Future<ChatSendResult> sendViaChain(String id) async {
    ...
    if (wire == null || message == null || !allowed || message.kind == ChatMessageKind.introduction) {
      return _failed('Chain fallback is only available after acceptance');
    }
    _isSending = true;
    _changed();
    try {
      ...
      if (!await _ownsBitNameNow(message.senderBitname)) {
        throw StateError('The sending BitName is no longer owned');
      }
      final remote = await bitnamesRPC.resolveBitName(message.recipientBitname);
      if (remote == null) {
        throw StateError('Recipient is not registered');
      }
      final pending = ChatMessage(
        id: message.id,
        content: message.content,
        senderBitname: message.senderBitname,
        recipientBitname: message.recipientBitname,
        timestamp: message.timestamp,
        isOutgoing: true,
        kind: message.kind,
        deliveryState: ChatDeliveryState.pending,
        transport: ChatTransport.bitnamesChain,
        introductionId: message.introductionId,
        valueSats: fallbackValueSats,
      );
      _storeMessageInMemory(pending);
      if (accepting) {
        _storeContactInMemory(
      ...
      final txid = await bitnamesRPC.transferIdempotent(
        idempotencyKey: id,
        dest: remote.address,
        value: fallbackValueSats,
        fee: minerFeeSats,
        memo: wire.ciphertext,
      );
      await _putMessage(pending.copyWith(txid: txid));
      return ChatSendResult(ChatSendDisposition.sent, id: id, reference: txid);
    } catch (e) {
      return ChatSendResult(ChatSendDisposition.needsChainConfirmation, id: id, error: '$e');
    } finally {
      _isSending = false;
      _changed();
    }
  ...
  Future<void> fetchPaymail() async {
    if (_polling || !bitnamesRPC.connected) {
      return;
    }
    _polling = true;
    try {
      ...
      for (final mail in mailbox) {
        for (final recipient in mail.recipients.where((r) => _ownedHashes.contains(r.bitname))) {
          try {
            if (await receiveWire(
              BitMessageWire(recipientBitNameHash: recipient.bitname, ciphertext: mail.output.memoHex),
              ChatTransport.bitnamesChain,
              value: mail.valueSats,
              requiredFee: recipient.requiredFeeSats,
              recipientData: recipient.data,
              reference: canonicalJsonEncode(mail.outpoint),
              blockHash: mail.blockHash,
              txIndex: mail.txIndex,
              validatedChainIds: validatedChainIds,
            )) {
              break;
            }
          } catch (e) {
            _fail('Could not receive chain message: $e');
          }
        }
      }
      await _reconcileChain(validatedChainIds);
    } catch (e) {
      _fail('Could not poll BitIntroduction messages: $e');
    } finally {
      _polling = false;
    }
  }
  ...
  Future<void> _confirmAcceptances() async {
    for (final message in _messages.where((m) => m.kind == ChatMessageKind.acceptance && m.txid != null)) {
      ...
      if (!{
        ...
        ChatRelationshipState.acceptancePending,
      }.contains(contact?.relationshipState)) {
        continue;
      }
      try {
        if (await bitnamesRPC.isTransactionConfirmed(message.txid!)) {
          await _putContact(contact!.copyWith(relationshipState: ChatRelationshipState.accepted));
        }
      } catch (_) {}
    }
  }
  ...
  Future<void> _recoverChainSubmissions() async {
    for (final message
        in _messages
            .where((m) => m.isOutgoing && m.transport == ChatTransport.bitnamesChain && _pendingWires.containsKey(m.id))
            .toList()) {
      try {
        final status = message.txid == null
            ? BitnamesTransactionStatus.absent
            : await bitnamesRPC.transactionStatus(message.txid!);
        if (status == BitnamesTransactionStatus.confirmed) {
          await _putMessage(message.copyWith(deliveryState: ChatDeliveryState.confirmed));
          continue;
        }
        if (status == BitnamesTransactionStatus.pending ||
            !await _mutationAllowed() ||
            !await _ownsBitNameNow(message.senderBitname)) {
          continue;
        }
        final remote = await bitnamesRPC.resolveBitName(message.recipientBitname);
        if (remote == null) {
        ...
        if (message.kind == ChatMessageKind.introduction && remote.data.paymailFeeSats != message.valueSats) {
          continue;
        }
        final txid = await bitnamesRPC.transferIdempotent(
          idempotencyKey: message.id,
          dest: remote.address,
          value: message.valueSats!,
          fee: minerFeeSats,
          memo: _pendingWires[message.id]!.ciphertext,
        );
  ...
  Future<void> _reconcileChain(Set<String> validatedChainIds) async {
    ...
    for (final message in auth) {
      try {
        if (message.txid != null &&
            (message.isOutgoing
                ? await bitnamesRPC.isTransactionConfirmed(message.txid!)
                : validatedChainIds.contains(message.id))) {
          valid.add(message.id);
        }
      } catch (_) {}
    }
    for (final message in auth.where((message) => message.isOutgoing)) {
      final state = valid.contains(message.id) ? ChatDeliveryState.confirmed : ChatDeliveryState.pending;
      if (message.deliveryState != state) {
        await _putMessage(message.copyWith(deliveryState: state));
      }
    }
    for (final contact in _contacts) {
      if ({ChatRelationshipState.blocked, ChatRelationshipState.rejected}.contains(contact.relationshipState)) {
        continue;
      }
      final intro = _messages
          .where(
            (m) =>
                m.id == contact.introductionId &&
                m.localBitname == contact.localBitname &&
                m.kind == ChatMessageKind.introduction,
          )
          .firstOrNull;
      if (intro == null) {
        continue;
      }
      final canonical = valid.contains(intro.id),
          acceptances = _messages.where(
            (m) =>
                m.localBitname == contact.localBitname &&
                m.kind == ChatMessageKind.acceptance &&
                m.introductionId == intro.id,
          );
      final accepted =
          canonical &&
          acceptances.any(
            (m) =>
                m.deliveryState == ChatDeliveryState.confirmed &&
                (m.transport != ChatTransport.bitnamesChain || valid.contains(m.id)),
          );
      final pending =
          canonical &&
          acceptances.any((m) => m.isOutgoing && m.transport == ChatTransport.bitnamesChain && !valid.contains(m.id));
      final state = accepted
          ? ChatRelationshipState.accepted
          : !canonical
          ? (intro.isOutgoing ? ChatRelationshipState.outgoingIntroduction : ChatRelationshipState.none)
          : intro.isOutgoing
          ? ChatRelationshipState.outgoingIntroduction
          : pending
          ? ChatRelationshipState.acceptancePending
          : ChatRelationshipState.incomingIntroduction;
      if (state != contact.relationshipState) {
        await _putContact(contact.copyWith(relationshipState: state));
      }
    }
    _messages.removeWhere((m) => auth.contains(m) && !m.isOutgoing && !valid.contains(m.id));
    await _save();
  }
  ...
  Future<bool> receiveWire(
    ...
    String? blockHash,
    int? txIndex,
    Set<String>? validatedChainIds,
  }) async {
    bool reject(String reason) {
      if (transport == ChatTransport.bitnamesChain) {
    ...
    if (!privateStorageReady) {
      return reject('private chat storage is locked or unavailable');
    }
    if (!_ownedHashes.contains(wire.recipientBitNameHash)) {
      return reject('recipient BitName is not owned');
    }
    final data = recipientData ?? (await bitnamesRPC.resolveBitName(wire.recipientBitNameHash))?.data;
    if (data?.encryptionPubkey == null || data?.signingPubkey == null) {
      return reject('recipient profile has no messaging keys');
    }
    ...
    if (utf8.encode(payload.body).length > maxMessageBytes) {
      return reject('message is oversized');
    }
    final existing = _find(payload.recipientBitNameHash, payload.senderBitNameHash);
    if (existing?.relationshipState == ChatRelationshipState.blocked) {
      return reject('sender is blocked');
    }
    if (payload.kind == BitIntroductionKind.introduction &&
        (transport != ChatTransport.bitnamesChain || requiredFee == null || value != requiredFee)) {
      return reject('introduction payment does not match the advertised fee');
    }
    if (payload.kind != BitIntroductionKind.introduction &&
        transport == ChatTransport.bitnamesChain &&
        value != fallbackValueSats) {
      return reject('fallback payment is not 1 sat');
    }
    final claimed = payload.replyProfile ?? _profile(existing);
    if (claimed == null) {
      return reject('sender supplied no reply profile');
    }
    final historical = blockHash == null || txIndex == null
        ? null
        : await bitnamesRPC.bitNameDataAtPosition(payload.senderBitNameHash, blockHash, txIndex);
    final sender = historical == null
        ? await _verifier.verify(claimed)
    ...
    final duplicate = _messages
        .where((m) => m.id == payload.id && m.localBitname == wire.recipientBitNameHash)
        .firstOrNull;
    if (duplicate != null) {
      final same =
          duplicate.senderBitname == payload.senderBitNameHash &&
          duplicate.recipientBitname == payload.recipientBitNameHash &&
          duplicate.content == payload.body &&
          duplicate.kind.index == payload.kind.index &&
          duplicate.timestamp.millisecondsSinceEpoch == payload.timestamp.millisecondsSinceEpoch;
      if (same) {
        validatedChainIds?.add(payload.id);
      }
      return same;
    }
    final state = switch (payload.kind) {
      BitIntroductionKind.introduction
          when existing == null ||
              existing.relationshipState == ChatRelationshipState.none ||
              existing.relationshipState == ChatRelationshipState.rejected =>
        ChatRelationshipState.incomingIntroduction,
      BitIntroductionKind.acceptance
          when existing?.relationshipState == ChatRelationshipState.outgoingIntroduction &&
    ...
    if (state == null) {
      return reject('message is invalid for the current relationship state');
    }
    final senderResolution = await bitnamesRPC.resolveBitName(payload.senderBitNameHash);
    if (senderResolution == null) {
      return reject('sender BitName is not registered');
    }
    final base =
        existing ??
        ChatContact(
          id: payload.senderBitNameHash,
          localBitname: payload.recipientBitNameHash,
          name: payload.senderBitNameHash,
          plaintextName: _allBitNames
              .where((entry) => entry.hash == payload.senderBitNameHash)
              .firstOrNull
              ?.plaintextName,
    ...
    _storeContactInMemory(
      base.copyWith(
        address: senderResolution.address,
        signingPubkey: sender.signingPublicKey,
        relationshipState: state,
        introductionId: payload.kind == BitIntroductionKind.introduction ? payload.id : existing?.introductionId,
        replyProfile: sender.profile.toJson(),
  ...
  Future<_Context?> _context() async {
    ...
    if (me == null || contact == null || !_ownedHashes.contains(me.hash) || contact.localBitname != me.hash) {
      return null;
    }
    final profile = _profiles[me.hash];
    if (profile == null || _pendingProfiles.contains(me.hash)) {
      _fail('Publish the reply profile in Messaging setup and wait for its BitNames block');
      return null;
    }
    try {
      if (!await _ownsBitNameNow(me.hash)) {
        throw StateError('The selected BitName is no longer owned');
      }
      final self = await _verifier.verify(profile);
      final remote = await bitnamesRPC.resolveBitName(contact.id);
      if (remote?.data.encryptionPubkey == null || remote?.data.signingPubkey == null) {
        return null;
      }
      return _Context(contact, self, remote!, _synthetic(contact.id, remote.data));
    } catch (e) {
      _fail('Could not verify messaging profiles: $e');
      return null;
    }
  }
  ...
  Future<(SignedBitIntroductionEnvelope, BitMessageWire)> _prepare(
    ...
    if (utf8.encode(body).length > maxMessageBytes) {
      throw ArgumentError('Messages are limited to $maxMessageBytes UTF-8 bytes');
    }
    final envelope = await _codec.createSigned(
      id: _newId(),
      kind: kind,
      senderProfile: ctx.self,
      recipientBitNameHash: ctx.contact.id,
      timestamp: _now(),
      body: body,
      inReplyTo: reply,
  ...
  VerifiedBitMessageProfile _synthetic(String hash, BitNameData data) {
    final profile = BitMessageProfile(
      bitNameHash: hash,
      signingPublicKey: data.signingPubkey!,
      encryptionPublicKey: data.encryptionPubkey!,
      paymailFeeSats: data.paymailFeeSats,
    );
    return VerifiedBitMessageProfile.verified(
      profile: profile,
      onChainCommitment: bitMessageProfileCommitment(profile),
      verificationReference: 'encryption-only',
    );
  }
  ...
  Future<bool> _ownsBitNameNow(String hash) async => (await bitnamesRPC.listUTXOs()).any((utxo) {
    if (utxo is! BitnamesUTXO || utxo.type != OutpointType.bitname) {
      return false;
    }
    try {
      return (jsonDecode(utxo.content) as Map<String, dynamic>)['BitName'] == hash;
    } catch (_) {
      return false;
    }
  });
  ...
  Iterable<ChatMessage> _conversation(String local, String remote) => _messages.where(
    (message) =>
        message.localBitname == local &&
  ...
  bool _profileMatchesCommitment(BitnameEntry identity, BitMessageProfile profile) => assessBitMessageProfileCommitment(
    profile: profile,
    onChainCommitment: identity.details.commitment,
    network: _commitmentNetwork,
  ).verified;
  ...
  ChatMessage _message(
    ...
    return ChatMessage(
      id: p.id,
      content: p.body,
      senderBitname: p.senderBitNameHash,
      recipientBitname: p.recipientBitNameHash,
      ...
      transport: transport,
      introductionId: p.kind == BitIntroductionKind.introduction ? p.id : p.inReplyTo,
      txid: reference,
      valueSats: value,
      error: error,
    );
  ...
  Future<bool> _mutationAllowed() async {
    ...
    if (_enforcer != null) {
      ...
      final stale = observedAt == null || _now().difference(observedAt) > pollInterval;
      if ((stale || !_chainHealth.mutationSafe) && !await _refreshChainHealth()) {
  ...
  List<ChatMessage> conversationPage({
    ...
    final messages =
        _conversation(
            localBitname,
            remoteBitname,
          ).where((message) => before == null || message.timestamp.isBefore(before)).toList()
  ...
  ChatSendResult _failed(String message) {
    _fail(message);
    return ChatSendResult(ChatSendDisposition.failed, error: message);
  }
...
class _Context {
  ...
  final VerifiedBitMessageProfile self, remoteCipher;
  final BitNameResolution remote;
}
...
String _defaultCommitmentNetwork() => _get<BitcoinConfProvider>()?.network.toReadableNet() ?? 'signet';
