...
class BitIntroductionPayload {
  BitIntroductionPayload({
    ...
    _boundedId(id, 'id');
    if (inReplyTo != null) {
      _boundedId(inReplyTo!, 'inReplyTo');
    }
...
String canonicalJsonEncode(Object? value) {
  ...
  if (value is num) {
    throw ArgumentError.value(value, 'value', 'canonical signed JSON only permits integer numbers');
  }
  if (value is List) {
...
List<Uri> _uriList(dynamic value, String field) {
  ...
  return value.map((item) {
    ...
    if (uri == null) {
      throw FormatException('$field contains an invalid URI');
    }
    return uri;
  }).toList();
}
