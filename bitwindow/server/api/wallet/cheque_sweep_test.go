...
func TestBuildAndSignChequeSweepTx(t *testing.T) {
	...
	unsignedTx, err := server.buildSweepTx(ctx, sourceAddress.EncodeAddress(), destAddress, testUTXOs, feeSatPerVbyte)
	if err != nil {
		t.Fatalf("build transaction: %v", err)
	}

	signedTx, err := server.signSweepTx(ctx, unsignedTx, testWIF, sourceAddress.EncodeAddress(), testUTXOs)
	if err != nil {
