...
func TestBitwindowWalletIntegration(t *testing.T) {
	...
	t.Run("Phase4_Cheques", func(t *testing.T) {
		...
		// =====================================================================
		// Phase 4b: Fund a cheque from the *enforcer* wallet
		//
		// Phase 4 above sends the funding tx from the bitcoinCore wallet. The
		// real-world Bitwindow flow on signet/testnet has the Enforcer Wallet
		// active when the user taps "Fund Check", so the orchestrator routes
		// SendTransaction through the enforcer's BDK wallet instead of
		// bitcoind's sendmany. This phase pins down that path end-to-end: the
		// BDK send must produce a tx that lands in bitcoind's mempool and the
		// cheque_watch descriptor wallet must see it via ListUnspent — which
		// is exactly what flips a check from "Unfunded" to "Funded" in the UI.
		// =====================================================================
		t.Log("--- Phase4b: Enforcer wallet funds a cheque ---")

		// Step 4b.1: fund the enforcer wallet so it has spendable balance.
		// Switch to the enforcer wallet to grab a fresh deposit address.
		_, err = nodeA.WalletClient.SwitchWallet(ctx, connect.NewRequest(&pb.SwitchWalletRequest{
			WalletId: enforcerID,
		}))
		require.NoError(t, err)

		enforcerAddrResp, err := nodeA.WalletClient.GetNewAddress(ctx, connect.NewRequest(&pb.GetNewAddressRequest{}))
		require.NoError(t, err)
		...
		fundEnforcerResp, err := nodeA.WalletClient.SendTransaction(ctx, connect.NewRequest(&pb.SendTransactionRequest{
			Destinations: map[string]int64{enforcerAddr: 100_000_000},
		}))
		require.NoError(t, err)
		t.Logf("funded enforcer wallet with tx: %s", fundEnforcerResp.Msg.Txid)

		err = nodeA.Mine(ctx, t, 1)
		require.NoError(t, err)

		// Wait for BDK to pick up the new block and report a positive balance.
		_, err = nodeA.WalletClient.SwitchWallet(ctx, connect.NewRequest(&pb.SwitchWalletRequest{
			WalletId: enforcerID,
		}))
		require.NoError(t, err)
		...
		// Step 4b.2: create a fresh cheque (uses the enforcer wallet's seed
		// for derivation — same descriptor space that was already imported
		// into cheque_watch earlier in Phase 4).
		createResp3, err := serverA.CreateCheque(ctx, connect.NewRequest(&walletpb.CreateChequeRequest{
			WalletId:           enforcerID,
			ExpectedAmountSats: chequeAmountSats,
		}))
		require.NoError(t, err)
		chequeID3 := createResp3.Msg.Id
		chequeAddr3 := createResp3.Msg.Address
		require.NotZero(t, chequeID3)
		require.True(t, strings.HasPrefix(chequeAddr3, "bcrt1"), "expected bcrt1 prefix, got %s", chequeAddr3)
		t.Logf("created enforcer-funded cheque #%d at %s", chequeID3, chequeAddr3)

		// Step 4b.3: with the enforcer wallet active, broadcast the funding tx
		// through the orchestrator — this is the exact RPC the Flutter
		// `check_detail_page.dart:fundWithWallet` call makes.
		_, err = nodeA.WalletClient.SwitchWallet(ctx, connect.NewRequest(&pb.SwitchWalletRequest{
			WalletId: enforcerID,
		}))
		require.NoError(t, err)

		enforcerFundResp, err := nodeA.WalletClient.SendTransaction(ctx, connect.NewRequest(&pb.SendTransactionRequest{
			Destinations:       map[string]int64{chequeAddr3: chequeAmountSats},
			FeeRateSatPerVbyte: 10,
		}))
		require.NoError(t, err, "enforcer-wallet send must succeed — this is what Fund Check calls")
		require.NotEmpty(t, enforcerFundResp.Msg.Txid)
		t.Logf("enforcer broadcast funding tx: %s", enforcerFundResp.Msg.Txid)

		// Step 4b.4: poll CheckChequeFunding before mining to verify the
		// watch wallet sees the unconfirmed broadcast. This is the polling
		// loop CheckProvider runs in Flutter; if it never flips to funded,
		// the UI sticks on "Unfunded" forever — the exact bug we're guarding.
		var enforcerFundingResp *connect.Response[walletpb.CheckChequeFundingResponse]
		for i := 0; i < 20; i++ {
			enforcerFundingResp, err = serverA.CheckChequeFunding(ctx, connect.NewRequest(&walletpb.CheckChequeFundingRequest{
				WalletId: enforcerID,
				Id:       chequeID3,
			}))
			require.NoError(t, err)
			if enforcerFundingResp.Msg.Funded {
				break
			}
			time.Sleep(500 * time.Millisecond)
		}
		require.True(
			t,
			enforcerFundingResp.Msg.Funded,
			"cheque_watch wallet must see the enforcer-wallet broadcast — if this fails, the Flutter Fund Check button stays stuck on Unfunded",
		)
		require.NotEmpty(t, enforcerFundingResp.Msg.FundedTxids)
		require.Equal(t, enforcerFundResp.Msg.Txid, enforcerFundingResp.Msg.FundedTxids[0])
		require.Equal(t, uint32(0), enforcerFundingResp.Msg.MinConfirmations, "should still be unconfirmed before mining")
		t.Logf("enforcer-funded cheque detected unconfirmed: %d sats, txid=%s",
			enforcerFundingResp.Msg.ActualAmountSats, enforcerFundingResp.Msg.FundedTxids[0])

		// Step 4b.5: mine and verify the confirmation count flips.
		err = nodeA.Mine(ctx, t, 1)
		require.NoError(t, err)

		confirmedResp, err := serverA.CheckChequeFunding(ctx, connect.NewRequest(&walletpb.CheckChequeFundingRequest{
			WalletId: enforcerID,
			Id:       chequeID3,
		}))
		require.NoError(t, err)
		require.True(t, confirmedResp.Msg.Funded)
		require.GreaterOrEqual(t, confirmedResp.Msg.MinConfirmations, uint32(1), "should have at least 1 confirmation after mining")
		t.Log("Phase4b passed — enforcer wallet → cheque_watch funding flow works end-to-end")
	})
}
