...
func newOrchestratorOnlyNode(t *testing.T) *orchestratorOnlyNode {
	...
	copyManagedBinary(t, filepath.Join(homeDir(t), "Library", "Application Support", "bitwindow", "assets", "bin", "bip300301-enforcer"), orchestrator.BinaryPath(bitwindowDir, "bip300301-enforcer"))
...
func prepareEnforcerConfigForSmokeTest(t *testing.T, rpcPort, jsonRPCPort, grpcPort int) {
	t.Helper()
	confDir := filepath.Join(homeDir(t), "Library", "Application Support", "bip300301_enforcer")
