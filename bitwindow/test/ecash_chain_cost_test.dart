...
void main() {
  ...
  // Without a plan the rollback block is unknown, so the steps must not tell a
  // user to switch: they would boot the target over the source fork's chain.
  test('the manual steps wait for a usable plan', () {
    final source = File('lib/widgets/ecash_upgrade_banner.dart').readAsStringSync();
    final steps = source.substring(source.indexOf('List<Widget> _manualSteps'));

    expect(steps, contains('if (rewindHeight == 0)'));
    expect(steps, contains('if (blocked)'));
  });
}
