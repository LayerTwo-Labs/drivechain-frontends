...
import 'package:sidechain_core/gen/bitnames/v1/bitnames.connect.client.dart';
import 'package:sidechain_core/gen/bitnames/v1/bitnames.pb.dart' as pb;
...
abstract class BitnamesRPC extends SidechainRPC {
  ...
  Future<BitNameData> bitNameDataAtPosition(
    String bitname,
    String blockHash,
    int txIndex,
  ) async {
    final data = await callRAW('bitname_data_at_position', [
      bitname,
      blockHash,
      txIndex,
    ]);
    if (data is! Map) {
      throw const FormatException('Invalid historical BitName data');
    }
  ...
  Future<List<PaymailEntry>> getPaymailEntries() async {
    ...
    if (entries is! List) {
      throw const FormatException('Invalid paymail entries');
    }
    return entries
        .map(
          (entry) => PaymailEntry.fromJson(Map<String, dynamic>.from(entry as Map)),
        )
        .toList();
  }
  ...
  Future<BitNameResolution?> resolveBitName(String bitname) async {
    ...
    if (resolution is! Map) {
      throw const FormatException('Invalid BitName resolution');
    }
  ...
  Future<String> updateBitName({
    ...
    final txid = await callRAW('update_bitname', [
      bitname,
      updates.toJson(),
      feeSats,
    ]);
    if (txid is! String) {
      throw const FormatException('Invalid BitName update transaction');
    }
    return txid;
  }
  ...
  Future<bool> verifySignature({
    ...
    final valid = await callRAW('verify_signature', [
      signature,
      verifyingKey,
      domain,
      msg,
    ...
    if (valid is! bool) {
      throw const FormatException('Invalid signature verification response');
    }
    return valid;
  }
  ...
  Future<String> transferIdempotent({
    ...
    final txid = await callRAW('transfer', [
      dest,
      value,
      fee,
      memo,
    ...
    if (txid is! String) {
      throw const FormatException('Invalid transfer transaction');
    }
    return txid;
  }
}
...
BitnamesTransactionStatus transactionInfoStatus(Object? info) {
  if (info == null) {
    return BitnamesTransactionStatus.absent;
  }
  if (info is! Map<String, dynamic>) {
    throw const FormatException('Invalid transaction info');
  }
  final txin = info['txin'];
  if (txin == null) {
    return BitnamesTransactionStatus.pending;
  }
