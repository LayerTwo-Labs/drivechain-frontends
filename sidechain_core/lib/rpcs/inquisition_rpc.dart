...
import 'package:sidechain_core/gen/inquisition/v1/inquisition.connect.client.dart';
import 'package:sidechain_core/gen/inquisition/v1/inquisition.pb.dart' as pb;
...
class InquisitionPegInfo {
  final bool synced;
  final String? mainchainTip;
  final String? lastError;

  const InquisitionPegInfo({required this.synced, this.mainchainTip, this.lastError});
}

/// API to the Inquisition node, served by the orchestrator.
///
...
abstract class InquisitionRPC extends SidechainRPC {
  InquisitionRPC({required super.binaryType});

  /// Whether the node's mirror of the enforcer is complete, and the mainchain
  /// tip it last saw.
  Future<InquisitionPegInfo> getPegInfo();
...
class InquisitionLive extends InquisitionRPC {
  @override
  final log = GetIt.I.get<Logger>();

  late InquisitionServiceClient _client;

  InquisitionLive() : super(binaryType: BinaryType.BINARY_TYPE_INQUISITION) {
    ...
    _client = InquisitionServiceClient(transport);
  }

  @override
  Future<List<String>> binaryArgs() async => ['-sidechainslot=${Inquisition().slot}'];

  @override
  Future<InquisitionPegInfo> getPegInfo() async {
    final resp = await _client.getSidechainInfo(pb.GetSidechainInfoRequest());
    return InquisitionPegInfo(
      synced: resp.synced,
  ...
  Future<BmmResult> mine(int feeSats) async {
    throw UnsupportedError(
      'Inquisition blocks are blind-merge-mined; the node holds no mainchain wallet',
    );
  }
  ...
  Future<dynamic> callRAW(String method, [dynamic params]) async {
    throw UnsupportedError('the inquisition node has no raw RPC passthrough');
  }
