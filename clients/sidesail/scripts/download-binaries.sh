...
if ! test -f $assets_dir/$drivechain; then
    echo Mainchain binary does not exist: $assets_dir/$drivechain
    ...
    for file in $drivechain $drivechain_cli; do 
        mv L1-Mainchain-latest-x86_64-$version_postfix/$file $assets_dir/$file
    done
fi 
...
if ! test -f $assets_dir/$sidechain; then
    echo Sidechain binary does not exist: $assets_dir/$sidechain
    ...
    if test -e $cli_file; then 
        mv $cli_file $assets_dir/$sidechain_cli
    fi 
fi;
