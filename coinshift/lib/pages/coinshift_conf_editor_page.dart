...
import 'package:coinshift/viewmodels/coinshift_config_editor_viewmodel.dart';

@RoutePage()
class CoinShiftConfEditorPage extends StatelessWidget {
  const CoinShiftConfEditorPage({super.key});

  @override
  Widget build(BuildContext context) {
    return ViewModelBuilder<CoinShiftConfigEditorViewModel>.reactive(
      viewModelBuilder: () => CoinShiftConfigEditorViewModel(),
      onViewModelReady: (viewModel) => viewModel.loadConfig(),
      builder: (context, viewModel, child) => _CoinShiftConfEditorPageContent(viewModel: viewModel),
    );
  }
}

class _CoinShiftConfEditorPageContent extends StatelessWidget {
  final CoinShiftConfigEditorViewModel viewModel;

  const _CoinShiftConfEditorPageContent({required this.viewModel});
                ...
                children: [
                  ...
                  Expanded(
                    child: SailText.secondary12(
                      'CoinShift does not read from a config file. These settings are converted to CLI arguments when CoinShift starts. '
                      'Restart CoinShift after saving for changes to take effect.',
                      color: theme.colors.info,
                    ),
                  ),
                ],
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          _TextSetting(
            label: 'RPC Address',
            currentValue: config.getSetting('rpc-addr') ?? '127.0.0.1:6255',
          ...
          _TextSetting(
            label: 'P2P Address',
            currentValue: config.getSetting('net-addr') ?? '0.0.0.0:4255',
...
class _CliPreview extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final theme = SailTheme.of(context);
    final confProvider = GetIt.I.get<CoinShiftConfProvider>();
        ...
        children: [
          ...
          Expanded(
            child: SelectableText(
              cliArgs.isEmpty ? '(no args)' : 'coinshift ${cliArgs.join(' ')}',
              style: TextStyle(
                fontFamily: 'IBMPlexMono',
                fontSize: 12,
                color: theme.colors.text,
