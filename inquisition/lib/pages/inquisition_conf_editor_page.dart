...
import 'package:inquisition/viewmodels/inquisition_config_editor_viewmodel.dart';

@RoutePage()
class InquisitionConfEditorPage extends StatelessWidget {
  const InquisitionConfEditorPage({super.key});

  @override
  Widget build(BuildContext context) {
    return ViewModelBuilder<InquisitionConfigEditorViewModel>.reactive(
      viewModelBuilder: () => InquisitionConfigEditorViewModel(),
      onViewModelReady: (viewModel) => viewModel.loadConfig(),
      builder: (context, viewModel, child) => _InquisitionConfEditorPageContent(viewModel: viewModel),
    );
  }
}

class _InquisitionConfEditorPageContent extends StatelessWidget {
  final InquisitionConfigEditorViewModel viewModel;

  const _InquisitionConfEditorPageContent({required this.viewModel});
                ...
                children: [
                  ...
                  Expanded(
                    child: SailText.secondary12(
                      'Inquisition does not read from a config file. These settings are converted to CLI arguments when Inquisition starts. '
                      'Restart Inquisition after saving for changes to take effect.',
                      color: theme.colors.info,
                    ),
                  ),
                ],
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          _TextSetting(
            label: 'RPC Address',
            currentValue: config.getSetting('rpc-addr') ?? '127.0.0.1:6009',
          ...
          _TextSetting(
            label: 'P2P Address',
            currentValue: config.getSetting('net-addr') ?? '0.0.0.0:4009',
        ...
        children: [
          ...
          Expanded(
            child: SailSelectableText(
              cliArgs.isEmpty ? '(no args)' : 'inquisition ${cliArgs.join(' ')}',
              monospace: true,
            ),
          ),
        ],
