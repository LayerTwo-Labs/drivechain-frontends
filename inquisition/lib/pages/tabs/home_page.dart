...
import 'package:inquisition/main.dart';
import 'package:inquisition/pages/tabs/settings_page.dart';
import 'package:inquisition/routing/router.dart';
import 'package:inquisition/utils/navigation_registry.dart';
...
enum Tabs {
  ...
  // inquisition configurable homepage
  InquisitionHomepage,
...
class _HomePageState extends State<HomePage> with WidgetsBindingObserver, WindowListener {
  ...
  InquisitionRPC get inquisitionRPC => GetIt.I.get<InquisitionRPC>();
  ...
  List<CommandItem> _getMenuCommands(BuildContext dialogContext) {
    ...
    return [
      ...
      CommandItem(
        ...
        onSelected: () async {
          await router.push(BackupWalletRoute(appName: 'inquisition'));
        },
      ),
      CommandItem(
          ...
          await router.push(
            SailCreateWalletRoute(
              ...
              additionalRestoreOptionsBuilder: (context) => WalletBackupRestoreOptions(
                ...
                binariesToStop: [BitcoinCore(), Enforcer(), Inquisition()],
              ),
            ),
          );
        },
      ...
      CommandItem(
        ...
        category: 'Navigation',
        onSelected: () {
          AutoRouterX(context).tabsRouter.setActiveIndex(Tabs.InquisitionHomepage.index);
          Navigator.of(dialogContext).pop();
        },
      ),
      CommandItem(
      ...
      CommandItem(
        ...
        category: 'Navigation',
        onSelected: () async {
          await router.push(InquisitionConfigureHomepageRoute());
        },
      ),
      ...
      // App menu
      CommandItem(
        label: 'Quit Inquisition',
        category: inquisitionRPC.chain.name,
        shortcut: '⌘Q',
    ...
    return Stack(
      children: [
        CrossPlatformMenuBar(
          menus: [
            PlatformMenu(
              label: inquisitionRPC.chain.name,
              menus: [
                PlatformMenuItemGroup(
                  members: [
                    PlatformMenuItem(
                      label: 'About ${inquisitionRPC.chain.name}',
                      onSelected: null,
                    ),
                  ],
                ),
                PlatformMenuItemGroup(
                  members: [
                    PlatformMenuItem(
                      label: 'Quit ${inquisitionRPC.chain.name}',
                    ...
                    PlatformMenuItem(
                      ...
                      onSelected: () async {
                        await GetIt.I.get<AppRouter>().push(
                          BackupWalletRoute(appName: 'inquisition'),
                        );
                      },
                    ),
                    PlatformMenuItem(
          ...
          child: SailScaffold(
            ...
            body: auto_router.AutoTabsRouter.builder(
              homeIndex: Tabs.InquisitionHomepage.index,
              routes: [
                // parent chain routes
                ParentChainRoute(),
                // inquisition configurable homepage
                InquisitionHomepageRoute(),
                // sidechain console route
                ConsoleTabRoute(),
                // trailing common routes
                SettingsTabRoute(),
                    ...
                    return SailScaffold(
                      ...
                      appBar: TopNav(
                        routes: [
                          ...
                          TopNavRoute(
                            label: 'Overview',
                            optionalKey: Tabs.InquisitionHomepage.index,
                            onTap: () {
                              tabsRouter.setActiveIndex(Tabs.InquisitionHomepage.index);
                            },
                          ),
                          TopNavRoute(
                            label: 'Console',
                        ...
                        endWidget: SailRow(
                          ...
                          children: [
                            ...
                            SailButton(
                              ...
                              small: true,
                              onPressed: () async {
                                await GetIt.I.get<AppRouter>().push(InquisitionConfigureHomepageRoute());
                              },
                            ),
                          ],
                        ),
                        ...
                        children: [
                          ...
                          BottomNav(
                            ...
                            additionalConnection: ConnectionMonitor(
                              rpc: inquisitionRPC,
                              name: inquisitionRPC.chain.name,
                            ),
                            ...
                            onOpenAdditionalConfConfigurator: () {
                              GetIt.I.get<AppRouter>().push(InquisitionConfEditorRoute());
                            },
                            endWidgets: [
                              ResetButton(
                                onTap: () async {
...
class HomePageViewModel extends BaseViewModel {
  ...
  InquisitionRPC get _rpc => GetIt.I.get<InquisitionRPC>();
