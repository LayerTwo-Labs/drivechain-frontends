...
import 'package:inquisition/models/inquisition_config.dart';

class InquisitionConfigEditorViewModel extends ChangeNotifier {
  final Logger log = GetIt.I.get<Logger>();
  final GenericSidechainConfProvider confProvider = GetIt.I.get<GenericSidechainConfProvider>();

  InquisitionConfig? originalConfig;
  InquisitionConfig? workingConfig;
  String? _rawConfigText;
  String? errorMessage;
  bool isLoading = false;
  bool _isDisposed = false;

  InquisitionConfigEditorViewModel() {
  ...
  Future<void> loadConfig() async {
    try {
      ...
      originalConfig = InquisitionConfig.parse(content);
      workingConfig = InquisitionConfig.fromConfig(originalConfig!);

      isLoading = false;
      notifyListeners();
    } catch (e) {
      log.e('Failed to load Inquisition config: $e');
  ...
  Future<void> saveConfig() async {
    ...
    try {
      ...
      originalConfig = InquisitionConfig.parse(configText);
      workingConfig = InquisitionConfig.fromConfig(originalConfig!);
      _rawConfigText = null;
      ...
      log.e('Failed to save Inquisition config: $e');
  ...
  void resetToDefault() {
    ...
    workingConfig = InquisitionConfig.parse(defaultContent);
    notifyListeners();
  }
  ...
  void updateFromRawText(String rawText) {
    ...
    try {
      workingConfig = InquisitionConfig.parse(rawText);
    } catch (e) {
      log.d('Inquisition config parsing failed during editing (this is ok): $e');
    }
