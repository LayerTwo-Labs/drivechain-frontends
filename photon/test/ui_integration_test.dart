    ...
    testWidgets('SailCard renders with title and content', (tester) async {
      await tester.pumpSailPage(
        SailCard(
          ...
          subtitle: 'Blockchain data explorer',
          child: SailText.primary15('Card Content'),
        ),
      );
      await tester.pumpAndSettle();

      expect(find.text('Photon Explorer'), findsOneWidget);
      expect(find.text('Blockchain data explorer'), findsOneWidget);
    ...
    testWidgets('SailTextField accepts input', (tester) async {
      ...
      await tester.pumpSailPage(
        SailTextField(
          ...
          hintText: 'Enter block hash or height',
        ),
      );
      await tester.pumpAndSettle();

      await tester.enterText(find.byType(TextField), '000000');
      expect(controller.text, '000000');
    });
  });
      ...
      await tester.pumpSailPage(
        StatefulBuilder(
          builder: (context, setState) => SailDropdownButton<String>(
            value: 'blocks',
            items: const [
              SailDropdownItem(value: 'blocks', label: 'Blocks'),
              SailDropdownItem(value: 'transactions', label: 'Transactions'),
            ],
            onChanged: (value) {},
          ),
        ),
