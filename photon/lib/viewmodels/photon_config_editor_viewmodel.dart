...
import 'package:photon/models/photon_config.dart';
import 'package:photon/providers/photon_conf_provider.dart';

class PhotonConfigEditorViewModel extends ChangeNotifier {
  final Logger log = GetIt.I.get<Logger>();
  final PhotonConfProvider confProvider = GetIt.I.get<PhotonConfProvider>();

  PhotonConfig? originalConfig;
  PhotonConfig? workingConfig;
  String? _rawConfigText;
  String? errorMessage;
  bool isLoading = false;
  bool _isDisposed = false;

  PhotonConfigEditorViewModel() {
  ...
  Future<void> loadConfig() async {
    try {
      ...
      originalConfig = PhotonConfig.parse(content);
      workingConfig = PhotonConfig.fromConfig(originalConfig!);

      isLoading = false;
      notifyListeners();
    } catch (e) {
      log.e('Failed to load Photon config: $e');
  ...
  Future<void> saveConfig() async {
    ...
    try {
      ...
      originalConfig = PhotonConfig.parse(configText);
      workingConfig = PhotonConfig.fromConfig(originalConfig!);
      _rawConfigText = null;
      ...
      log.e('Failed to save Photon config: $e');
  ...
  void resetToDefault() {
    ...
    workingConfig = PhotonConfig.parse(defaultContent);
    notifyListeners();
  }
  ...
  void updateFromRawText(String rawText) {
    ...
    try {
      workingConfig = PhotonConfig.parse(rawText);
    } catch (e) {
      log.d('Photon config parsing failed during editing (this is ok): $e');
    }
