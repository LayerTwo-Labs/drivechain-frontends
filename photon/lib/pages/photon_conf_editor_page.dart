...
import 'package:photon/viewmodels/photon_config_editor_viewmodel.dart';

@RoutePage()
class PhotonConfEditorPage extends StatelessWidget {
  const PhotonConfEditorPage({super.key});

  @override
  Widget build(BuildContext context) {
    return ViewModelBuilder<PhotonConfigEditorViewModel>.reactive(
      viewModelBuilder: () => PhotonConfigEditorViewModel(),
      onViewModelReady: (viewModel) => viewModel.loadConfig(),
      builder: (context, viewModel, child) => _PhotonConfEditorPageContent(viewModel: viewModel),
    );
  }
}

class _PhotonConfEditorPageContent extends StatelessWidget {
  final PhotonConfigEditorViewModel viewModel;

  const _PhotonConfEditorPageContent({required this.viewModel});
                ...
                children: [
                  ...
                  Expanded(
                    child: SailText.secondary12(
                      'Photon does not read from a config file. These settings are converted to CLI arguments when Photon starts. '
                      'Restart Photon after saving for changes to take effect.',
                      color: theme.colors.info,
                    ),
                  ),
                ],
...
class _CliPreview extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final theme = SailTheme.of(context);
    final confProvider = GetIt.I.get<PhotonConfProvider>();
        ...
        children: [
          ...
          Expanded(
            child: SelectableText(
              cliArgs.isEmpty ? '(no args)' : 'photon ${cliArgs.join(' ')}',
              style: TextStyle(
                fontFamily: 'IBMPlexMono',
                fontSize: 12,
                color: theme.colors.text,
