...
void main() {
  ...
  test('light mode says it raises on the enforcer bids', () {
    mode.setMode(wmpb.NodeMode.NODE_MODE_LIGHT);

    expect(model.isLight, isTrue);
    expect(bidSourceInLightMode, contains('raises up to the ceiling'));
    expect(bidSourceInLightMode, contains('rival bids from the enforcer'));
  });
