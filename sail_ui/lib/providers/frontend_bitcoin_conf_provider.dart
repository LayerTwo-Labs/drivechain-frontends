...
/// Frontend (file-based) implementation of BitcoinConfProvider.
/// Used by BitWindow and sidechain apps that read/write bitcoin.conf directly.
class FrontendBitcoinConfProvider extends BitcoinConfProvider {
  ...
  Future<void> _restartServicesForNetworkChange() async {
    ...
    _updateMainchainRPCConfig(readMainchainConf());
  ...
  /// The downstream file is read-only - any changes should be made to the master file in BitWindow directory.
  Future<void> _copyConfigDownstream() async {
    try {
      ...
      // Read source content and prepend read-only header
...
''';
      ...
      await destFile.writeAsString(headerComment + sourceContent);
