...
/// Frontend (file-based) implementation of EnforcerConfProvider.
/// Used by BitWindow and sidechain apps that read/write enforcer config directly.
class FrontendEnforcerConfProvider extends EnforcerConfProvider {
  ...
  FrontendEnforcerConfProvider._create();

  static Future<FrontendEnforcerConfProvider> create() async {
    final instance = FrontendEnforcerConfProvider._create();
  ...
  Future<void> loadConfig() async {
    try {
      ...
      if (await file.exists()) {
        ...
        final migrated = runConfigMigrations<EnforcerConfig>(
          config,
          _kEnforcerConfVersion,
          (c) => c.configVersion,
          (c, v) => c.configVersion = v,
          _enforcerConfMigrations,
        );
        if (migrated) {
