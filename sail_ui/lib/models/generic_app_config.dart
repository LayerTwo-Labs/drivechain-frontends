/// Generic configuration model for sidechain apps.
...
class GenericAppConfig {
  ...
  GenericAppConfig({required this.appName});

  GenericAppConfig.fromConfig(GenericAppConfig other)
    : appName = other.appName,
      settings = Map<String, String>.from(other.settings);

  /// Parse a simple key=value format config file (no sections)
  static GenericAppConfig parse(String content, {required String appName}) {
    final config = GenericAppConfig(appName: appName);
  ...
  String serialize() {
    ...
    buffer.writeln('# $appName Configuration - Generated by $appName App');
    buffer.writeln('# These settings are converted to CLI arguments when $appName starts.');
    buffer.writeln();
