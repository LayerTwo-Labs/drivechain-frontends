...
import 'package:bbc/viewmodels/bbc_config_editor_viewmodel.dart';

@RoutePage()
class BbcConfEditorPage extends StatelessWidget {
  const BbcConfEditorPage({super.key});

  @override
  Widget build(BuildContext context) {
    return ViewModelBuilder<BbcConfigEditorViewModel>.reactive(
      viewModelBuilder: () => BbcConfigEditorViewModel(),
      onViewModelReady: (viewModel) => viewModel.loadConfig(),
      builder: (context, viewModel, child) => _BbcConfEditorPageContent(viewModel: viewModel),
    );
  }
}

class _BbcConfEditorPageContent extends StatelessWidget {
  final BbcConfigEditorViewModel viewModel;

  const _BbcConfEditorPageContent({required this.viewModel});
                ...
                children: [
                  ...
                  Expanded(
                    child: SailText.secondary12(
                      'Bbc does not read from a config file. These settings are converted to CLI arguments when Bbc starts. '
                      'Restart Bbc after saving for changes to take effect.',
                      color: theme.colors.info,
                    ),
                  ),
                ],
        ...
        children: [
          ...
          Expanded(
            child: SailSelectableText(
              cliArgs.isEmpty ? '(no args)' : 'bbc ${cliArgs.join(' ')}',
              monospace: true,
            ),
          ),
        ],
