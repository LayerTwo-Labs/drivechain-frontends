...
import 'package:bbc/config/runtime_args.dart';
import 'package:bbc/providers/bbc_conf_provider.dart';
import 'package:bbc/providers/bbc_homepage_provider.dart';
import 'package:bbc/routing/router.dart';
...
Future<(Directory, File, Logger)> init(String arguments) async {
  ...
  await initSidechainDependencies(
    sidechainType: BinaryType.BINARY_TYPE_BBC,
    createSidechainConnection: (_) {
      if (GetIt.I.isRegistered<BbcRPC>()) {
        bbcRPC = GetIt.I.get<BbcRPC>();
      } else {
        bbcRPC = BbcLive();
        GetIt.I.registerSingleton<BbcRPC>(bbcRPC);
      }
      return bbcRPC;
    },
  ...
  unawaited(
    initBackendManagedSidechainRuntime(
      log: log,
      binary: BinaryType.BINARY_TYPE_BBC,
      appRpc: bbcRPC,
    ),
  );

  // Initialize BbcConfProvider (must be after BitcoinConfProvider)
  final bbcConfProvider = await BbcConfProvider.create();
  if (!GetIt.I.isRegistered<GenericSidechainConfProvider>()) {
    GetIt.I.registerLazySingleton<GenericSidechainConfProvider>(() => bbcConfProvider);
  }

  // Register homepage provider
  final bbcHomepageProvider = BbcHomepageProvider();
  if (!GetIt.I.isRegistered<BbcHomepageProvider>()) {
    GetIt.I.registerLazySingleton<BbcHomepageProvider>(() => bbcHomepageProvider);
  }
  ...
  if (!GetIt.I.isRegistered<HomepageProvider>()) {
    GetIt.I.registerLazySingleton<HomepageProvider>(() => bbcHomepageProvider);
  }
...
Future<void> runMultiWindow(
  ...
  log.i('starting bbc in multi window');

  return runApp(
    buildSailWindowApp(
      log,
      '${arguments['window_title'] as String} | Bbc',
      child,
      bbc.chain.color,
    ),
  );
...
void bootBinaries(Logger log) {
  unawaited(
    bootBackendManagedSidechain(
      log: log,
      binary: BinaryType.BINARY_TYPE_BBC,
      appRpc: GetIt.I.isRegistered<BbcRPC>() ? GetIt.I.get<BbcRPC>() : null,
    ),
  );
}
...
Future<void> runMainWindow(Logger log, Directory applicationDir, File logFile) async {
  ...
  runApp(
    SailApp(
      dense: false,
      builder: (context) {
        return _BbcAppContent(
          router: router,
          bbc: bbc,
        );
      },
      accentColor: bbc.chain.color,
      log: log,
    ),
  );
}

class _BbcAppContent extends StatelessWidget {
  final AppRouter router;
  final BbcRPC bbc;

  const _BbcAppContent({
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp.router(
      ...
      title: bbc.chain.name,
      theme: ThemeData(
        fontFamily: theme.chrome.fontFamily ?? (font == SailFontValues.ibmMono ? 'IBMPlexMono' : 'Inter'),
        colorScheme: ColorScheme.fromSwatch().copyWith(secondary: bbc.chain.color),
      ),
    );
  }
}
...
void _installSignalShutdownHandlers(Logger log) {
  ...
  Future<void> handleSignal(ProcessSignal signal) async {
    ...
    log.w('received ${signal.toString()}, shutting down Bbc runtime');
...
void _installAppExitHandler(Logger log) {
  ...
  _appLifecycleListener = AppLifecycleListener(
    onExitRequested: () async {
      ...
      _signalShutdownInProgress = true;
      log.w('received app exit request, shutting down Bbc runtime');
...
Future<void> initAutoUpdater(Logger log) async {
  ...
  try {
    const feedURL = 'https://releases.drivechain.info/appcast-bbc.xml';
