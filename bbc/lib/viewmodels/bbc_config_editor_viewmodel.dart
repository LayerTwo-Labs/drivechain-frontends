...
import 'package:bbc/models/bbc_config.dart';

class BbcConfigEditorViewModel extends ChangeNotifier {
  final Logger log = GetIt.I.get<Logger>();
  final GenericSidechainConfProvider confProvider = GetIt.I.get<GenericSidechainConfProvider>();

  BbcConfig? originalConfig;
  BbcConfig? workingConfig;
  String? _rawConfigText;
  String? errorMessage;
  bool isLoading = false;
  bool _isDisposed = false;

  BbcConfigEditorViewModel() {
  ...
  Future<void> loadConfig() async {
    try {
      ...
      originalConfig = BbcConfig.parse(content);
      workingConfig = BbcConfig.fromConfig(originalConfig!);

      isLoading = false;
      notifyListeners();
    } catch (e) {
      log.e('Failed to load Bbc config: $e');
  ...
  Future<void> saveConfig() async {
    ...
    try {
      ...
      originalConfig = BbcConfig.parse(configText);
      workingConfig = BbcConfig.fromConfig(originalConfig!);
      _rawConfigText = null;
      ...
      log.e('Failed to save Bbc config: $e');
  ...
  void resetToDefault() {
    ...
    workingConfig = BbcConfig.parse(defaultContent);
    notifyListeners();
  }
  ...
  void updateFromRawText(String rawText) {
    ...
    try {
      workingConfig = BbcConfig.parse(rawText);
    } catch (e) {
      log.d('Bbc config parsing failed during editing (this is ok): $e');
    }
