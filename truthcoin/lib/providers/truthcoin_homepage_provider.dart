...
class TruthcoinHomepageConfigurationSetting extends SettingValue<HomepageConfiguration> {
  @override
  String get key => 'truthcoin_homepage_configuration';

  TruthcoinHomepageConfigurationSetting({super.newValue});

  @override
  HomepageConfiguration defaultValue() => TruthcoinHomepageConfiguration.defaultConfiguration;
  ...
  SettingValue<HomepageConfiguration> withValue([HomepageConfiguration? value]) {
    return TruthcoinHomepageConfigurationSetting(newValue: value);
  }
}

class TruthcoinHomepageConfiguration {
...
class TruthcoinHomepageProvider extends HomepageProvider {
  final ClientSettings _settings = GetIt.I.get<ClientSettings>();

  @override
  HomepageConfiguration configuration = TruthcoinHomepageConfiguration.defaultConfiguration;
  @override
  HomepageConfiguration tempConfiguration = TruthcoinHomepageConfiguration.defaultConfiguration;
  @override
  bool isLoading = false;
  @override
  bool hasUnsavedChanges = false;

  TruthcoinHomepageProvider() {
    _loadConfiguration();
  }
  ...
  Future<void> _loadConfiguration() async {
    ...
    try {
      final setting = TruthcoinHomepageConfigurationSetting();
      final loadedSetting = await _settings.getValue(setting);
      configuration = loadedSetting.value;
      tempConfiguration = configuration;
    } catch (e) {
      configuration = TruthcoinHomepageConfiguration.defaultConfiguration;
      tempConfiguration = configuration;
    } finally {
      isLoading = false;
      notifyListeners();
  ...
  Future<void> saveConfiguration() async {
    ...
    try {
      final setting = TruthcoinHomepageConfigurationSetting(newValue: tempConfiguration);
  ...
  Map<String, HomepageWidgetInfo> getWidgetCatalog() {
    return TruthcoinWidgetCatalog.getCatalogMap();
  }
}
